...
- **Queue methods available**: `pause`, `resume`, `shuffle`, `jump`, `seek`, `setRepeatMode`, `toggleAutoplay`, `addToQueue`, `skip`, `stop`, `previous`. Feature work uses these; no reimplementation. **Correction (phase 7 critique): `Queue.remove()` takes NO argument — it deletes the whole queue from the manager.** There is no single-song removal API; `remove <n>` splices `queue.songs` directly (bypassing the task queue, so it must not run concurrently with other controls from different surfaces).
...
- **YouTube playback uses yt-dlp (live-test correction, 2026-08-17).** ytdl-core (official and the `ytdl-core-stuff` fork, even at its latest commit) fails on current YouTube: `Error applying decipher: no solutions`, and many videos return zero playable formats. `src/music/yt-dlp-plugin.ts` spawns the `yt-dlp` binary directly (latest release, on PATH, `--no-warnings` so warnings can't pollute the JSON) for search (`ytsearch1:`), URL/playlist resolution, and audio download. The `@distube/yt-dlp` wrapper was rejected: it merges stderr into its JSON parse, so any yt-dlp warning (deprecated `--no-call-home` on 2026.07.04, "older than 90 days", "no JS runtime") breaks it.
- **Audio is downloaded to a file, not streamed.** googlevideo audio URLs (itag 251/140) answer **only closed-range requests** (open-ended or no Range → 403), which ffmpeg cannot stream continuously. The plugin downloads `-f ba/ba*` to `$TMPDIR/dj-audio/<id>.<ext>` and returns a `file://` URL; files older than a day are cleaned up.
- **YouTube's `android` player client is used, not the default `android_vr`.** The `android_vr` media URLs get IP-throttled (403 on the byte download) after a burst of requests, while `android` is unaffected and yields a progressive mp4 (itag 18) that ffmpeg reads directly. One yt-dlp invocation per song keeps the bot well under YouTube's throttling.
