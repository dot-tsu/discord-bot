...
- **No ytdl-core fork.** The `ToddyTheNoobDud/ytdl-core-stuff` git dependency's entire delta over official `@distube/ytdl-core` is one line: `playerClients` defaulting to `['ANDROID_VR', 'WEB_EMBEDDED', 'IOS', 'ANDROID', 'TV']`. That default is already configurable, so the fork is deleted and the same clients are set in `distube-config.js` via `new YouTubePlugin({ ytdlOptions: { playerClients: [...] } })`. If playback works without it once ffmpeg exists, drop the override too.
