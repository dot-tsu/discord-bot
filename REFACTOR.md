...
- **Audio is downloaded to a file, not streamed.** googlevideo audio URLs (itag 251/140, `c=ANDROID_VR`) answer **only closed-range requests** (open-ended or no Range → 403), which ffmpeg cannot stream continuously. The plugin downloads `-f ba/ba*` to `$TMPDIR/dj-audio/<id>.<ext>` and returns a `file://` URL; files older than a day are cleaned up. One yt-dlp invocation per song keeps the bot well under YouTube's IP throttling (triggered by bursts of requests).
