...
- **The ytdl-core fork IS required (correction, live test 2026-08-17).** The phase-0 conclusion that `ToddyTheNoobDud/ytdl-core-stuff` could be dropped was wrong. Live playback with the official `@distube/ytdl-core@4.16.12` fails: `Error applying decipher: no solutions` / `Error applying n transform: no solutions`, and restricted videos throw `Failed to find any playable formats` (all player clients return empty streaming data). The fork's maintained `lib/ejs-solvers.js` (JS challenge solver, built from a git submodule) is what keeps it working against current YouTube. Pin it to the **latest commit** (currently `5c779c07`, with the pre-built solver); the old pinned SHA `bf801ac` is stale. Do not re-add `ytdlOptions.playerClients` overrides.
